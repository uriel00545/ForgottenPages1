...
public class BossFinalController {
    ...
    @FXML private ImageView imgBoss;
    ...
    public void initialize() {
        ...
        log("DESAFIO FINAL");
        log("O Rei Esqueleto desperta...");
        setStatus("Enfrente o Rei Esqueleto! Escolha sua skill.");
    }


    private void configurarImagensPersonagens() {
        definirImagem(imgBoss, "reiesqueleto.png");
    ...
    private Boss criarBossFinal() {
        Boss b = new Boss(new BasicAi(), "Rei Esqueleto", 500, 500);
    ...
    private void verificarFase() {
        ...
        if (bossFinal.getHp() <= limiar && fase < 3) {
            ...
            log("FASE " + fase + "! O Rei Esqueleto ficou mais forte!");
    ...
    private void verificarEstado() {
        if (!player.Tavivo()) {
            log("Você foi derrotado pelo Rei Esqueleto...");
            gs.finalizarRun();
            Navegar.ir("end");
            return;
        }
        ...
        if (!bossFinal.Tavivo()) {
            log("VITÓRIA! O Rei Esqueleto foi derrotado!");
