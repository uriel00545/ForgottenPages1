    ...
    private void upgradeVida() {
        comprar("vida", () -> {
            ...
            if (gs.getProgressao().getVidaNivel() < MAX_NIVEL) {
                gs.getPlayer().setMaxHp(gs.getPlayer().getMaxHp() + BONUS_VIDA);
                gs.getPlayer().setHp(gs.getPlayer().getHp() + BONUS_VIDA);
    ...
    private void upgradeClash() {
        comprar("clash", () -> {
            ...
            if (gs.getProgressao().getClashNivel() < MAX_NIVEL) {
                int proximoNivel = gs.getProgressao().getClashNivel() + 1;

                if (proximoNivel % INTERVALO_BONUS_CLASH == 0) {
                    gs.getPlayer().getSkills()
                            .forEach(s -> s.setCoinPower(s.getCoinPower() + 1));
                }

                gs.getProgressao().setClashNivel(proximoNivel);
            }
        });
    }
